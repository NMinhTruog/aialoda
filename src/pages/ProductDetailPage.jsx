...
import { buildProductSchema, buildWebPageSchema } from '@/seo/schema';
...
function ProductDetailPage() {
    ...
    if (product && selectedVariant) {
      ...
      try {
        ...
        toast({
          ...
          description: `${quantity} x ${product.title} (${selectedVariant.title}) da duoc them vao gio.`,
        });
      } catch (error) {
        toast({
          variant: 'destructive',
    ...
    return (
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center text-red-400 p-8 glass-card rounded-2xl">
          ...
          <p className="mb-6">Khong the tai thong tin goi tu van: {error}</p>
        </div>
      </div>
    );
  }
  ...
  const pageTitle = `${product.title} | Aloda`;
  const pageDescription = product.subtitle || product.title;
  ...
  return (
    <>
      <Helmet>
        ...
        <link rel="canonical" href={getAbsoluteUrl(`/product/${product.id}`)} />
      </Helmet>
      <SeoSchema
        schemas={[
          buildWebPageSchema({
      ...
      />
      <div className="max-w-5xl mx-auto">
        <Link to="/shop" className="inline-flex items-center gap-2 text-white hover:text-purple-300 transition-colors mb-6">
          <ArrowLeft size={16} />
          Quay lai danh sach
        </Link>
        <div className="grid md:grid-cols-2 gap-8 glass-card p-8 rounded-2xl">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="flex flex-col">
            ...
            {product.variants.length > 1 && (
              <div className="mb-6">
                <h3 className="text-sm font-medium text-white mb-2">Lua chon</h3>
