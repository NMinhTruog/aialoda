  ...
  const handleAddToCart = useCallback(async (e) => {
    ...
    try {
      ...
      toast({
        ...
        description: `${product.title} da duoc dua vao gio quan tam cua ban.`,
      });
    } catch (error) {
      toast({
      ...
      <Link to={`/product/${product.id}`}>
        <div className="rounded-lg border bg-card text-card-foreground shadow-sm glass-card border-0 text-white overflow-hidden group transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
          ...
          <div className="p-4">
            ...
            <p className="text-sm text-gray-300 h-10 overflow-hidden">{product.subtitle || 'Noi dung tu van va giai phap se duoc cap nhat tai day.'}</p>
            <Button onClick={handleAddToCart} className="w-full mt-4 bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white font-semibold">
              <ShoppingCart className="mr-2 h-4 w-4" /> Dat lich tu van
            </Button>
          </div>
        </div>
      </Link>
  ...
  if (error) {
    return (
      <div className="text-center text-red-400 p-8">
        <p>Khong the tai danh sach giai phap: {error}</p>
      </div>
    );
  }
  ...
  if (products.length === 0) {
    return (
      <div className="text-center text-gray-400 p-8">
        <p>Hien chua co goi tu van nao duoc dang.</p>
      </div>
    );
  }
